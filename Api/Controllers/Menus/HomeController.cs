...
public class HomeController(IResponseControler responseControler,
    ...
    ITipoPostRepository tipoPostRepository) : BaseAuthorizeController(responseControler)
{
    [HttpPost, Route("getTable")]
    public async Task GetRequest(CancellationToken cancellationToken)
    {
        if (!responseControler.IsAuthenticated)
            responseControler.SetResponseData(await repository.GetTableAsync(Models.Menu.ELiberado.Sim, cancellationToken));
        else
